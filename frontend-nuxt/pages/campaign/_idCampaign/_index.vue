...
export default {
  ...
  beforeMount() {
    ...
    this.$axios
      .$get(process.env.API_DEV_URL + `campaign/${this.$route.params.idCampaign}`)
      .then((resp) => {
        let list_idMembers = [];
        ...
        if (resp.campaigns.campaign_members.length !== 1) {
          for (let i = 0; i < resp.campaigns.campaign_members.length; i++) {
            if (resp.campaigns.campaign_members[i].is_host === 1) {
              resp.campaigns.host_name = resp.campaigns.campaign_members[i].users.name;
              resp.campaigns.id_host = resp.campaigns.campaign_members[i].user_id;
              resp.campaigns.list_members = resp.campaigns.campaign_members.splice(i, 1);
            }
          }
        } else {
          resp.campaigns.host_name = resp.campaigns.campaign_members[0].users.name;
