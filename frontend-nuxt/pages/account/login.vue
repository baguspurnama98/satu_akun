    ...
    <div class="flex items-center justify-center mt-8">
        <div class="w-full max-w-md py-5 px-3">
            <form @submit.prevent="login" class="bg-gray-100 shadow-lg rounded-md px-8 xs:px-4 pt-6 pb-8 mb-4 mx-4 ">
                ...
                <div class="relative">
                  <input placeholder="" :type="hiddenPass ? 'password' : 'text'" class="appearance-none border rounded w-full py-3 px-3 text-gray-900 focus:border-indigo-500 focus:outline-none" placeholder="Password" name="password" required autocomplete="current-password" 
                    v-model="form.password" @keydown="errorStatus = false">
                  <div class="absolute inset-y-0 right-0 pr-5 flex items-center text-sm  ">

                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" @click="hiddenPass = !hiddenPass" :class="[hiddenPass == true? 'hidden' : 'block']" >
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                  </svg>
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" @click="hiddenPass = !hiddenPass" :class="[hiddenPass == false? 'hidden' : 'block']">
